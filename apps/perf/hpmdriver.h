...
#define SET(reg, field, value)                                                 \
  (reg) = ((reg) & ~((uint64_t)(field##_MASK) << (field##_OFFSET))) |          \
          ((uint64_t)(value) << (field##_OFFSET));
...
#define print_event(id)                                                        \
  printf("mhpmevent%d: %lx\n", id, syscall(GET_PERF, EVENT_BASE + id))
#define print_counter(id)                                                      \
  printf("mhpmcounter%d: %lu\n", id, syscall(GET_PERF, COUNTER_BASE + id))
...
#define set_event_quad(csr_id, mode, optype2, optype1, optype0, event3,        \
                       event2, event1, event0)                                 \
  {                                                                            \
...
#define set_event_double(csr_id, mode, optype0, event1, event0)                \
  set_event_quad(csr_id, mode, OPTYPE_OR, OPTYPE_OR, optype0, noEvent,         \
                 noEvent, event1, event0)

#define set_event_single(csr_id, mode, event)                                  \
  set_event_quad(csr_id, mode, OPTYPE_OR, OPTYPE_OR, OPTYPE_OR, noEvent,       \
                 noEvent, noEvent, event)

// set event and clear counter
#define se_cc_quad(csr_id, mode, optype2, optype1, optype0, event3, event2,    \
                   event1, event0)                                             \
  {                                                                            \
    set_event_quad(csr_id, mode, optype2, optype1, optype0, event3, event2,    \
                   event1, event0);                                            \
    clear_counter(csr_id);                                                     \
  }
#define se_cc_double(csr_id, mode, optype0, event1, event0)                    \
  {                                                                            \
    set_event_double(csr_id, mode, optype0, event1, event0);                   \
    clear_counter(csr_id);                                                     \
  }
#define se_cc_single(csr_id, mode, event)                                      \
  {                                                                            \
    set_event_single(csr_id, mode, event);                                     \
    clear_counter(csr_id);                                                     \
  }
